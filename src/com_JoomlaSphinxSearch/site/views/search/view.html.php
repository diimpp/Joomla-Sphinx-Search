...
class SphinxSearchViewSearch extends JView
{
    public function display($tpl = null)
    {
        ...
        if ($app->getCfg('sitename_pagetitles', 0) == 1) {
            $title = JText::sprintf(
                'JPAGETITLE', $app->getCfg('sitename'), $title
            );
        ...
        if ($prmtrs) {
            ...
            // Get sublayout.
            $subLayoutPath = $this->_getLayoutPath(
                $prmtrs['extension'], $prmtrs['template'], $prmtrs['layout']
            );
            $subLayout = 'default';
        ...
        //    JURI::base() . 'media/com_sphinxsearch/css/sphinxsearch.css'
        //);
    ...
    private function _getMenuAlias(JMenuSite $menus, $extension, $template)
    {
        ...
        foreach ($menu as $menuCh) {
            if ('index.php?option=' . $extension  . '&view='
